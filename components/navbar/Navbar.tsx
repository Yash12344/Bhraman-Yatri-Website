...
interface NavbarProps {
  /** Trek links for the "All Treks" dropdown, passed in from server data. */
  trekLinks: { label: string; href: string }[];
}

export function Navbar({ trekLinks }: NavbarProps) {
    ...
    >
      ...
      >
        ...
        <ul className="hidden items-center gap-8 lg:flex">
          {NAV_LINKS.map((link) => {
            const hasDropdown = link.href === "/treks" && trekLinks.length > 0;
            return (
              <li key={link.href} className={cn(hasDropdown && "group relative")}>
                ...
                {hasDropdown && (
                  <ul className="invisible absolute left-1/2 top-full z-50 w-64 -translate-x-1/2 translate-y-2 rounded-xl border border-neutral-100 bg-white p-2 opacity-0 shadow-xl transition-all duration-300 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
                    {trekLinks.map((trek) => (
                      <li key={trek.href}>
                        <Link
                          href={trek.href}
      ...
      <div
        ...
        className={cn(
          ...
          isMobileOpen ? "max-h-[520px]" : "max-h-0 border-t-0"
        )}
      >
