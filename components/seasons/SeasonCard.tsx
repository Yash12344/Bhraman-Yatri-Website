...
export function SeasonCard({ season }: SeasonCardProps) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-gray-100 bg-white shadow-[0_10px_30px_-14px_rgba(0,0,0,0.18)] transition-shadow duration-300 hover:shadow-[0_18px_40px_-16px_rgba(0,0,0,0.28)]">
      <h3
        className={cn(
          "flex h-12 items-center justify-center gap-2 text-[15px] font-bold uppercase tracking-wide text-white",
          isWinter ? "bg-forest-800" : "bg-saffron-500"
        )}
      >
        <HeaderIcon aria-hidden="true" className="size-4.5" />
        {season.title}
      </h3>

      <ul className="grid flex-1 grid-cols-1 gap-x-8 gap-y-5 p-6 sm:grid-cols-2">
      ...
      <Link
        ...
        className="flex items-center justify-center gap-1 border-t border-gray-100 bg-gray-50 py-3.5 text-sm font-semibold uppercase tracking-wide text-gray-900 transition-colors hover:text-saffron-500"
      >
        {season.viewAllLabel}
