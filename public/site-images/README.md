...
| `bhrigu-lake.jpg` | Bhrigu Lake Trek |
...
Most pictures in this folder are **temporary placeholders**, generated by
`scripts/generate-placeholders.mjs` because the build environment had no
internet access to fetch stock photography. Replace them with real photographs
using the same file names.

Two exceptions are real photographs, taken from the operator's own brochures
and web pages. Neither is in the placeholder generator, so regenerating cannot
overwrite them:

- **Bhrigu Lake** — `treks/bhrigu-lake.jpg` plus the four images in
  `treks/gallery/bhrigu-lake/`.
- **Dayara Bugyal** — `treks/dayara-bugyal-trek.jpg`, taken from
  `DAYARA_BUGYAL_TREK__HIMALAYAN_AMBITION.pdf` after the client reported the
  previous placeholder reading as a blank panel.
